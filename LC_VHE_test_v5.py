...
class take_data:
	...
	# This neat little formula saved me a bit of time. Rather than do the calculation and write a new csv file, this does the calculations for the Normalization values N0 here.
		for F, F_err, I in zip(PF, PF_err, PI):
			Norm = [((F/(E2-E1))*((E2**(1+I)-E1**(1+I))/(1+I)))]; self.Norm = Norm
			Norm_P = [(((F+F_err)/(E2-E1))*((E2**(1+I)-E1**(1+I))/(1+I)))]; self.Norm_P = Norm_P
			Norm_M = [(((F-F_err)/(E2-E1))*((E2**(1+I)-E1**(1+I))/(1+I)))]; self.Norm_M = Norm_M
	...
	# A dictionary so that we can refer to specific time bins later
		...
		for tb, i, v in zip(time_bin, Norm, PI):
			...
			One_GeV.append(np.interp(1,myVOT['{}'.format(tb)].VS.EBins, myVOT['{}'.format(tb)].VS.dNdE_absorbed))
			Four_GeV.append(np.interp(400,myVOT['{}'.format(tb)].VS.EBins, myVOT['{}'.format(tb)].VS.dNdE_absorbed))
			One_TeV.append(np.interp(1000,myVOT['{}'.format(tb)].VS.EBins, myVOT['{}'.format(tb)].VS.dNdE_absorbed))
		...
		for tb, i, v in zip(time_bin, Norm_P, PI):
			...
			dNdE_new_p = [np.interp(10**myVOT_PFP['{}'.format(tb)].VR.EASummary['minSafeE'], myVOT_PFP['{}'.format(tb)].VS.EBins, myVOT_PFP['{}'.format(tb)].VS.dNdE_absorbed)] 
			...
			One_GeV_P.append(np.interp(1,myVOT_PFP['{}'.format(tb)].VS.EBins, myVOT_PFP['{}'.format(tb)].VS.dNdE_absorbed)); self.One_GeV_P = One_GeV_P
			Four_GeV_P.append(np.interp(400,myVOT_PFP['{}'.format(tb)].VS.EBins, myVOT_PFP['{}'.format(tb)].VS.dNdE_absorbed));	self.Four_GeV_P = Four_GeV_P
			One_TeV_P.append(np.interp(1000,myVOT_PFP['{}'.format(tb)].VS.EBins, myVOT_PFP['{}'.format(tb)].VS.dNdE_absorbed)); self.One_TeV_P = One_TeV_P
			
			# Integrates differential flux with respect to energy bins using the trapezoid rule
			int_flux_p = [np.trapz(dNdE_new_p, x = E_new_p)]; self.int_flux_p = int_flux_p
			
			#checks how long detection takes
			
			crabFlux_p = 100*myVOT_PFP['{}'.format(tb)].rate*60./myVOT_PFP['{}'.format(tb)].VR.crabRate
			crab_flux_p = [myVOT_PFP['{}'.format(tb)].VR.crabRate]; self.crab_flux_p = crab_flux_p
			detTime_P = np.interp([crabFlux_p*0.01], myVOT_PFP['{}'.format(tb)].VR.SensCurve[:,0], myVOT_PFP['{}'.format(tb)].VR.SensCurve[:,1])*60
			detTimes_p = [detTime_P[0]]; self.detTimes_p = detTimes_p
		...
		for tb, i, v in zip(time_bin,Norm_M, PI):
			...
			# interpolates the differential flux at 1 GeV, 400 GeV and 1 TeV 
			One_GeV_M = [np.interp(1,myVOT_PFM['{}'.format(tb)].VS.EBins, myVOT_PFM['{}'.format(tb)].VS.dNdE_absorbed)]
			Four_GeV_M = [np.interp(400,myVOT_PFM['{}'.format(tb)].VS.EBins, myVOT_PFM['{}'.format(tb)].VS.dNdE_absorbed)]
			One_TeV_M = [np.interp(1000,myVOT_PFM['{}'.format(tb)].VS.EBins, myVOT_PFM['{}'.format(tb)].VS.dNdE_absorbed)]
			...
			detTimes_m = [detTime_m[0]); self.detTimes_m = detTimes_m
		
		
		for err, perr in zip(int_flux, int_flux_p):
			perror = [perr - err]; self.perror= perror
		
		
		for err, merr in zip(int_flux, int_flux_m):
			merror = [err-merr]; self.merror = merror
		...
		for tb, i, v in zip(time_bin, Norm_P, PIP):
			...
			# interpolates the differential flux at 1 GeV, 400 GeV and 1 TeV 
			One_GeV_PI = [np.interp(1,myVOT_PIP['{}'.format(tb)].VS.EBins, myVOT_PIP['{}'.format(tb)].VS.dNdE_absorbed)]; self.One_GeV_PI = One_GeV_PI
			Four_GeV_PI = [np.interp(400,myVOT_PIP['{}'.format(tb)].VS.EBins, myVOT_PIP['{}'.format(tb)].VS.dNdE_absorbed)]; self.Four_GeV_PI = Four_GeV_PI
			One_TeV_PI = [np.interp(1000,myVOT_PIP['{}'.format(tb)].VS.EBins, myVOT_PIP['{}'.format(tb)].VS.dNdE_absorbed)]; self.One_TeV_PI = One_TeV_PI
			
			# Integrates differential flux with respect to energy bins using the trapezoid rule
			int_flux_pi = [np.trapz(dNdE_new_pi, x = E_new_pi)]; self.int_flux_pi = int_flux_pi
			
			
			#checks how long detection takes
			crabFlux_pi = 100*myVOT_PIP['{}'.format(tb)].rate*60./myVOT_PIP['{}'.format(tb)].VR.crabRate
			crab_flux_pi = [myVOT_PIP['{}'.format(tb)].VR.crabRate]
			detTime_PI = np.interp([crabFlux_pi*0.01], myVOT_PIP['{}'.format(tb)].VR.SensCurve[:,0], myVOT_PIP['{}'.format(tb)].VR.SensCurve[:,1])*60
			detTimes_pi = [detTime_PI[0]]
		...
		for tb, i, v in zip(time_bin,Norm_M, PIM):
			...
			# interpolates the differential flux at 1 GeV, 400 GeV and 1 TeV 
			One_GeV_MI = [np.interp(1,myVOT_PIM['{}'.format(tb)].VS.EBins, myVOT_PIM['{}'.format(tb)].VS.dNdE_absorbed)]; self.One_GeV_MI = One_GeV_MI
			Four_GeV_MI = [np.interp(400,myVOT_PIM['{}'.format(tb)].VS.EBins, myVOT_PIM['{}'.format(tb)].VS.dNdE_absorbed)]; self.Four_GeV_MI = Four_GeV_MI
			One_TeV_MI = [np.interp(1000,myVOT_PIM['{}'.format(tb)].VS.EBins, myVOT_PIM['{}'.format(tb)].VS.dNdE_absorbed)]; self.One_TeV_MI = One_TeV_MI
			
			# Integrates differential flux with respect to energy bins using the trapezoid rule
			int_flux_mi = [np.trapz(dNdE_new_mi, x = E_new_mi)]; self.int_flux_mi = int_flux_mi
			
			#checks how long detection takes
			
			crabFlux_mi = 100*myVOT_PIM['{}'.format(tb)].rate*60./myVOT_PIM['{}'.format(tb)].VR.crabRate
			crab_flux_mi = [myVOT_PIM['{}'.format(tb)].VR.crabRate]; self.crab_flux_mi = crab_flux_mi
			detTime_MI = np.interp([crabFlux_mi*0.01], myVOT_PIM['{}'.format(tb)].VR.SensCurve[:,0], myVOT_PIM['{}'.format(tb)].VR.SensCurve[:,1])*60
			detTimes_mi = [detTime_MI[0]]; self.detTimes_mi = detTimes_mi
